...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(15.0, 5.0),  # All numbers are approximate.
                rg.Point(7.5, 9.3),
                rg.Point(7.5, 0.7)]
    circle = rg.Circle(rg.Point(10, 5), 5)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  5 aqua dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(100, 100), 50)
    draw_points_on_circle(window, circle, 5, 'aqua')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(generate_points_on_circle(circle, number_of_points))):
        c = rg.Circle(generate_points_on_circle(circle, number_of_points)[k], 10)
        c.fill_color = color
        c.attach_to(window)
        window.render()
    # ------------------------------------------------------------------
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 4:  100 slices on yellow circle, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(generate_points_on_circle(circle,number_of_segments))-1):
        l = rg.Line(generate_points_on_circle(circle,number_of_segments)[k],generate_points_on_circle(circle,number_of_segments)[k+1])
        l.color = color
        l.thickness = thickness
        l.attach_to(window)

    l = rg.Line(generate_points_on_circle(circle,number_of_segments)[len(generate_points_on_circle(circle,number_of_segments))-1],generate_points_on_circle(circle,number_of_segments)[0])
    l.color = color
    l.thickness = thickness
    l.attach_to(window)
    window.render()
